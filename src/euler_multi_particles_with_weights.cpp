...
arma::mat update_weights(
  ...
  for(int i =0; i < steps; i++) {
    ...
    for(int p = 0; p < weights_init.n_rows; p++) {
      ...
      for (int v = 0; v < d_unob; v++) {
        eval_env.assign(as<std::string>(unobserved_vars[v]), X(p, v, i * simulations_per_weight_update));
      }
...
Rcpp::List euler_multi_particles_with_weights(
  ...
  arma::mat weights = update_weights(
      X, r, weight_init, time_var, t0, dt,
      unobserved_vars, simulations_per_weight_update, steps,
      observed_drift, observed_diffusion, deltaY, eval_env);
