...
changeFixedParametersToConstant <- function(yuima, fixed) {
  ...
  new.drift.func <- function(index, unfixed.vals, state.vals) {
    transform.drift(yuima@model@drift, fixed, drift.unfixed.nms, state.nms, index, unfixed.vals, state.vals)
  }
  new.diffusion.func <- function(row, index, unfixed.vals, state.vals) {
    transform.diffusion(yuima@model@diffusion, fixed, diffusion.unfixed.nms, state.nms, row, index, unfixed.vals, state.vals)
  }
  ...
  if (length(yuima@model@jump.coeff) > 0) {
    ...
    new.jump.func <- function(row, index, unfixed.vals, state.vals) {
      transform.jump(yuima@model@jump.coeff, fixed, jump.unfixed.nms, state.nms, row, index, unfixed.vals, state.vals)
    }
    vector.jump <- c()
  ...
  if (class(yuima@model) == "yuima.linear_state_space_model") {
    new.ymodel <- setModel(
      ...
      xinit = yuima@model@xinit, observed.variable = yuima@model@state.variable[yuima@model@is.observed]
    )
  } else {
    new.ymodel <- setModel(
      drift = transformed.drift, diffusion = transformed.diffusion, hurst = yuima@model@hurst,
